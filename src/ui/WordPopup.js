...
export class WordPopup {
  ...
  async show(word, selectionRect, contextData = {}) {
  ...
  renderLookupResult(response, word, content, header, lookupType) {
  ...
  renderError(word, response, header, content) {
    const errorText = document.createElement('span');
  ...
  renderDefinition(response, word, content, header) {
  ...
  createWordHeader(word, synonymsList, sourceType) {
  ...
  appendSynonyms(headerContainer, synonymsList) {
  ...
  createDefinitionsContainer(definitions) {
  ...
  createDefinitionBlock(def, index, totalDefs) {
  ...
  renderTranslation(response, content, header) {
    const wordSpan = document.createElement('span');
    wordSpan.style.cssText = `
      font-size: 14px;
      font-weight: 600;
